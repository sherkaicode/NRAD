#!/bin/bash
#SBATCH --job-name=Eval_NRAD_2
#SBATCH --partition=tartarus
#SBATCH --output=Eval_NRAD_2.txt
#SBATCH -e e_Eval_NRAD_2.txt
#SBATCH --ntasks=8
#SBATCH --nodes=1
...
do
	...
	do	
		...
		python CWoLa_SR.py -s $sig_inj -g $i -reweight -generate -morph
		
	done
	echo "Train CWoLa for CR (How well the background is created if signal = 0)"
	python CWoLa_CR.py -g $i -reweight -generate -morph
done
